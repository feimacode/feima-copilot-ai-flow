...
Defines the `extensionPack` and `extensionDependencies` declarations in `package.json` that ensure `feima.copilot-more-llms` (Feima global model provider) and `github.copilot-chat` are installed alongside `feima.copilot-ai-flow`.
